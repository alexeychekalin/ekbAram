                ...
                <div class="form-container uk-overflow-auto">
                    <table class="uk-table uk-table-hover uk-table-middle uk-text-center uk-table-divider" style="background-color: white;">
                        ...
                        <tr>
                            ...
                            <th class="uk-table-middle uk-text-center">Terms</th>
                            <th class="uk-table-middle uk-text-center">Credit Line</th>
...
export default {
    ...
    methods:{
        ...
        show(param){
            ...
            this.creditline = param.creditline
            //this.ipo = param.ipo
        },
        updateСlients(param){
            this.show2 = true
            axios.post('/api/clients/update', {
                ...
                .then(res => {
                    this.show2 = false
                    UIkit.notification({message: 'Customer info updated'})
        ...
        deleteClients(param, cnt, name){
            UIkit.modal.confirm('Please confirm you wish to remove supplier "' + name + '"').then((answer)=> {
                ...
                axios.post('/api/clients/delete', {id: param, name: 'null', address: 'null', email: 'null', code:'null'})
                    .then(res => {
                        this.show2 = false
                        UIkit.notification({message: 'Customer deleted'})
                        this.results.splice(cnt,1)
                        this.allresults.splice(cnt,1)
                    }).catch(({response:{data}})=>{
                    UIkit.notification({message: 'Error delete Customer. This customer has another orders'})
                }).finally(()=>{
                    UIkit.modal("#modal-change").hide()
                    this.getClients();
                })
