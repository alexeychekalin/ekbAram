...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
  ...
  methods: {
    ...
    OtherFileUpload: function OtherFileUpload(e) {
      var _this5 = this;
      ...
      axios.post('/api/fileupload/other/' + localStorage.getItem('changeid'), formData, config).then(function (res) {
        _this5.show2 = false;
    ...
    getProvider: function getProvider() {
      ...
      axios.get('/api/provider').then(function (res) {
        _this8.$data.providers = res.data;
      });
    },
    ...
    createOrder: function createOrder() {
      ...
      this.orders.forEach(function (el, i) {
        if (!_this9.parts.find(function (e) {
          return e.pn.toLowerCase() === el.part.toLowerCase();
        })) {
          axios.post('/api/parts', {
            pn: el.part,
            description: _this9.descriptions[i].description
          }).then(function (res) {
            UIkit.notification({
              message: 'Новая позиция добавлена!',
              status: 'success'
            });
            _this9.orders[i].part = res.data.id;
          })["catch"](function (error) {
            UIkit.notification({
              message: error,
              status: 'danger'
          ...
          _this9.orders[i].part = _this9.parts.find(function (el) {
            return el.pn === _this9.orders[i].part;
          }).id;
        }
      ...
      if (this.newaddress !== '') {
        ...
        axios.post('/api/clients/address', {
          ...
          _this9.clients[_this9.client].address3 = _this9.shipto;
          _this9.address3 = _this9.shipto;
        })["catch"](function (error) {
          this.show2 = false;
          UIkit.notification({
            message: error,
      ...
      if (this.fileChanged) {
        ...
        axios.post('api/fileupload', formData, config).then(function (res) {
          _this9.ipo = res.data.success; // create order

          axios.post('api/order/update', {
            number: _this9.number,
            datestart: _this9.timeStart,
            dateend: _this9.timeStop,
            ipo: _this9.ipo,
            client: _this9.clients[_this9.client].id,
            shipto: _this9.shipto,
            manager: _store__WEBPACK_IMPORTED_MODULE_6__["default"].state.auth.user.id,
            delivery: _this9.delivery,
            comission: _this9.comission,
            currency: _this9.currency,
            terms: _this9.terms,
            id: _this9.$route.params.id,
            wd: _this9.wd
          }).then(function (response) {
            // create order list
            _this9.orders.forEach(function (el) {
              el.order_number = _this9.$route.params.id;
            });

            axios.post('api/orderlist', _this9.$data.orders).then(function (response) {
              UIkit.notification({
  ...
  mounted: function mounted() {
    ...
    axios.get('/api/order/' + this.$route.params.id).then(function (res) {
      //заполняем заказ
      _this10.number = res.data[0][0].number;
      _this10.timeStart = res.data[0][0].datestart;
      _this10.timeStop = res.data[0][0].dateend;
      _this10.ipo = res.data[0][0].ipo;
      _this10.client = _this10.clients.findIndex(function (x) {
        return x.id === res.data[0][0].client;
      });

      _this10.selectClient();

      _this10.shipto = res.data[0][0].shipto;
      _this10.currency = res.data[0][0].currency;
      _this10.filename = res.data[0][0].ipo;
      _this10.delivery = res.data[0][0].delivery;
      _this10.delivery = res.data[0][0].comission;
      _this10.terms = res.data[0][0].terms;
      _this10.wd = res.data[0][0].wd; // заполняем позиции
  ...
  return _c(
    ...
    [
      ...
      _c(
        ...
        [
          _c("h1", { staticClass: "uk-text-center" }, [_vm._v("Edit Entry")]),
          _vm._v(" "),
          _c(
            "div",
            {
                          ...
                          _c(
                            ...
                            [
                              ...
                              _c("div", { attrs: { "uk-form-custom": "" } }, [
                                _c("input", {
                                  attrs: { type: "file" },
                                  on: { change: _vm.OtherFileUpload },
                                }),
                                _vm._v(" "),
                                _c("span", { staticClass: "uk-link" }, [
                                  _vm._v("Tap to select files"),
                                ]),
                              ]),
                            ]
                          ),
