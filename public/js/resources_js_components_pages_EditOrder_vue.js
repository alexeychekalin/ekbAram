...
/* harmony default export */ const __WEBPACK_DEFAULT_EXPORT__ = ({
        ...
        axios.post('api/fileupload', formData, config).then(function (res) {
          ...
          axios.post('api/order/update', {
            ...
            _this6.orders.forEach(function (el) {
              el.order_number = _this6.$route.params.id;
            });
  ...
  mounted: function mounted() {
    ...
    axios.get('/api/order/' + this.$route.params.id).then(function (res) {
      //заполняем заказ
      _this7.number = res.data[0][0].number;
      _this7.timeStart = res.data[0][0].datestart;
      _this7.timeStop = res.data[0][0].dateend;
      _this7.ipo = res.data[0][0].ipo;
      _this7.client = _this7.clients.findIndex(function (x) {
        return x.id === res.data[0][0].client;
      });

      _this7.selectClient();

      _this7.shipto = res.data[0][0].shipto;
      _this7.currency = res.data[0][0].currency;
      _this7.filename = res.data[0][0].ipo;
      _this7.delivery = res.data[0][0].delivery;
      _this7.delivery = res.data[0][0].comission; // заполняем позиции
